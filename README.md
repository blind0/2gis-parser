...
Перед парсингом организаций необходимо собрать список городов и рубрик:

```bash
cd app

# Собрать список всех городов
scrapy crawl cities

# Собрать список всех рубрик
scrapy crawl rubrics
```

Результат сохраняется в `cities_full.json` и `rubrics_full.json`.
...
Код предоставляется **только для некоммерческого использования**. Подробнее — в [LICENSE.md](LICENSE.md).
